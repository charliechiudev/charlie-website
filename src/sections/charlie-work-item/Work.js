...
const Work = ({ portfolioItem }) => {
  ...
  return (
    <PageWrapper>
      <Container fluid className="mt-5 pt-5 px-sm-5 px-xs-0">
        {mainImage && (
          ...
          <video loop muted autoPlay playsInline className="w-100"><source src={mainImage.node.publicURL} type="video/mp4" /></video> 
        )}
      </Container>
              ...
              {gallery.map((item, i) => (
                <Col lg={"12"} md={"12"} sm={"12"} className="mb-4" key={`gallery-${i}`}>
                  {Array.isArray(item) ?
                    ...
                    <video key={`gallery-${i}`} loop muted autoPlay playsInline className="w-100"><source src={item.node.publicURL} type="video/mp4" /></video>
                  }
                </Col>
              ))}
            </Row>
